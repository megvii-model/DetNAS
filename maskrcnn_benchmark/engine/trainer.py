    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        if 'search' in cfg.MODEL.BACKBONE.CONV_BODY:
            ...
            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                blocks_keys = model.module.blocks_keys
                num_states = model.module.num_states
            else:
                blocks_keys = model.blocks_keys
                num_states = model.num_states
            rngs = np.random.randint(
                low=0, high=len(blocks_keys), size=(num_states))
            rngs = broadcast_data(rngs.tolist())
